    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh]">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-black italic uppercase text-emerald-600 mb-4">เกมแปลไทย</h1>
          <p className="text-slate-600 font-bold mb-2">เลือกคำแปลที่ถูกต้อง</p>
        ...
        <button 
          ...
          className="bg-emerald-500 text-white px-12 py-4 rounded-[2rem] shadow-xl font-black text-xl italic uppercase transform active:scale-95 transition-all"
        >
          🎮 Start Game
        </button>
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="grid grid-cols-2 gap-3 w-full max-w-sm px-4">
        {options.map((opt, idx) => (
          <button key={idx} onClick={() => handleAnswer(opt)} className="bg-white p-4 rounded-2xl border-b-4 border-slate-200 active:border-0 active:translate-y-1 transition-all shadow-sm font-black text-slate-700 italic uppercase text-sm">
            {opt.th}
          </button>
        ))}
      </div>
