...
export default function MiniGames_th({ user, allMasterCards, selectedIds, timerSetting, setPage }) {
  ...
  const initGame = useCallback(async () => {
    ...
    const storageKey = `playedWords_th_${mode}`;
  ...
  useEffect(() => {
    ...
    if (!gameStarted && user && user.id) {
      console.log('[MiniGames_th] useEffect triggered, user.id:', user.id);
      
      // คำนวณ reviewCountDisplay ทันที (ไม่ต้องรอคลิก Review)
      const fetchReviewDisplay = async () => {
        try {
          console.log('[MiniGames_th] Starting fetchReviewDisplay...');
          ...
          if (error) {
            console.error('[MiniGames_th] Error fetching review count:', error);
          ...
          if (!reviewData) {
            console.warn('[MiniGames_th] No review data returned');
            setReviewCountDisplay(0);
            setReviewCount(0);
            return;
          }
          ...
          console.log('[MiniGames_th] Review poolIds:', poolIds.length);
          ...
          try {
            ...
            console.error('[MiniGames_th] Error reading localStorage:', e);
            reviewPlayedWords = [];
          }
          
          console.log('[MiniGames_th] Review playedWords:', reviewPlayedWords.length);
          ...
          console.log('[MiniGames_th] Review calculation:', {
          ...
          console.log('[MiniGames_th] Setting reviewCountDisplay:', displayCount);
          
          setReviewCountDisplay(displayCount);
          setReviewCount(poolIds.length);
        } catch (error) {
          console.error('[MiniGames_th] Error in fetchReviewDisplay:', error);
          // Set fallback value
          setReviewCountDisplay(0);
          setReviewCount(0);
        }
      ...
      console.log('[MiniGames_th] User not ready yet, waiting...');
    }
  ...
  const handleAnswer = async (selected) => {
    ...
    if (isCorrect) {
      ...
      // กรณี Review Mode
      if (mode === 'review') {
        // อัพเดท minigame_wrong_count แยกตาม game_type 'th' (reset เป็น 0)
      ...
      if (mode === 'review') {
        ...
        const reviewStorageKey = `playedWords_th_review`;
        const reviewPlayedWords = JSON.parse(localStorage.getItem(reviewStorageKey) || '[]');
        const validPlayedWords = reviewPlayedWords.filter(id => {
          // กรองเฉพาะคำที่ยังอยู่ใน pool (minigame_wrong_count.th > 0)
          return true; // จะกรองใน initGame ครั้งถัดไป
        });
        const remaining = newCount - validPlayedWords.length;
        setReviewCountDisplay(remaining > 0 ? remaining : (newCount > 0 ? newCount : 0));
  ...
  if (!gameStarted) {
    ...
    const normalStorageKey = `playedWords_th_normal`;
