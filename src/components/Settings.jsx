      ...
      <div className="space-y-6 pt-4">
        ...
        <div className="bg-white p-6 rounded-[2.5rem] border-2 border-slate-100 shadow-sm space-y-6">
          <div className="text-center">
            <label className="block text-xs font-black text-slate-400 mb-2 uppercase">Timer (Seconds)</label>
            <div className="flex items-center justify-center gap-4">
              <button 
                onClick={() => { const s = Math.max(1, timerSetting - 1); setTimerSetting(s); saveSettings(s, schedules); }} 
                className="w-10 h-10 bg-slate-100 rounded-full font-black text-xl shadow-sm"
              >-</button>
              <div className="text-3xl font-black text-orange-600 italic w-16">{timerSetting}s</div>
              <button 
                onClick={() => { const s = timerSetting + 1; setTimerSetting(s); saveSettings(s, schedules); }} 
                className="w-10 h-10 bg-slate-100 rounded-full font-black text-xl shadow-sm"
              >+</button>
            </div>
          </div>
          <button onClick={() => setPage('select-words')} className="w-full bg-orange-500 text-white p-4 rounded-3xl font-black uppercase italic shadow-lg shadow-orange-100">
            📂 Select Study Words
          </button>
          <button onClick={() => setPage('set-schedule')} className="w-full bg-slate-800 text-white p-4 rounded-3xl font-black uppercase italic shadow-lg">
            📅 Set Level Schedule
          </button>
        </div>
      </div>
  ...
  if (page === 'set-schedule') {
    return (
      <div className="space-y-4 pt-2">
            ...
            {daysOfWeek.map(day => (
              <button 
                ...
                onClick={() => {
                  ...
                  setSchedules(newS); saveSettings(timerSetting, newS);
                }} 
        ...
        {/* Monthly Schedule */}
        <div className="bg-white p-5 rounded-3xl border-2 border-slate-100 space-y-4 shadow-sm mb-10">
          ...
          <div className="grid grid-cols-6 gap-2 p-1">
            {datesOfMonth.map(date => (
              <button 
                ...
                className={`text-sm p-2.5 rounded-xl font-black border-2 transition-all ${schedules.lv5.includes(date) ? 'bg-purple-500 text-white border-purple-500 shadow-md shadow-purple-100' : schedules.lv6.includes(date) ? 'bg-emerald-500 text-white border-emerald-500 shadow-md shadow-emerald-100' : 'bg-slate-50 text-black border-slate-100'}`}
              >
                {date}
              </button>
            ))}
