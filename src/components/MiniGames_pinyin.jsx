...
export default function MiniGames_pinyin({ user, allMasterCards, selectedIds, timerSetting, setPage }) {
  ...
  // ฟังก์ชันดึง Review count จาก DB (แยกตาม game_type: pinyin)
  const fetchReviewCount = async () => {
    ...
    // ถ้า minigame_wrong_count เป็น JSON object ให้เช็ค game_type 'pinyin'
    ...
    const count = data.filter(item => {
      if (typeof item.minigame_wrong_count === 'object' && item.minigame_wrong_count !== null) {
        return (item.minigame_wrong_count.pinyin || 0) > 0;
      }
  ...
  const initGame = useCallback(async () => {
    ...
    if (mode === 'review') {
      ...
      // กรองเฉพาะคำที่มี minigame_wrong_count > 0 สำหรับ game_type 'pinyin'
  ...
  useEffect(() => {
    if (!gameStarted && user?.id) {
      (async () => {
        ...
        const normalPlayedWords = await getPlayedIds(user.id, 'pinyin', 'normal');
  ...
  const loadNextQuestion = (id, currentQueue) => {
    if (!id) { 
      clearPlayedIds(user.id, 'pinyin', mode);
  ...
  const handleAnswer = async (selected) => {
    ...
    if (isCorrect) {
      ...
      if (!playedWords.includes(questionId)) {
        await setPlayedIds(user.id, 'pinyin', mode, [...playedWords, questionId]);
      }
      ...
      if (mode === 'review') {
        ...
        if (typeof prog?.minigame_wrong_count === 'object' && prog?.minigame_wrong_count !== null) {
          newWrongCount = { ...prog.minigame_wrong_count, pinyin: 0 };
        } else {
          newWrongCount = { pinyin: 0 };
        }
        let updateOk = false;
      ...
      setShowFeedback(true);
      playSound('wrong');
      saveWrongWord(user.id, questionId, 'pinyin', currentQuestion);
      ...
      // อัพเดท minigame_wrong_count แยกตาม game_type 'pinyin'
      let newWrongCount = {};
      if (typeof prog?.minigame_wrong_count === 'object' && prog?.minigame_wrong_count !== null) {
        newWrongCount = { ...prog.minigame_wrong_count, pinyin: (prog.minigame_wrong_count.pinyin || 0) + 1 };
      } else {
        // ถ้าเป็น number เก่า ให้แปลงเป็น object
        const oldCount = prog?.minigame_wrong_count || 0;
        newWrongCount = { pinyin: oldCount + 1 };
      }
    ...
    >
      <div className="w-full flex justify-between items-center mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <button onClick={async () => { const qId = currentQuestion?.id1 ?? currentQuestion?.id; if (qId != null) await saveWrongWord(user.id, qId, 'pinyin', currentQuestion); setWrongWordToast('ได้เพิ่มคำผิดไว้ใน list ให้แล้ว ดูรายการได้ที่ Settings'); setTimeout(() => setWrongWordToast(null), 2500); }} className="bg-amber-500 text-white px-2 py-1 rounded-full font-black text-[10px] italic uppercase">คำผิด</button>
        </div>
