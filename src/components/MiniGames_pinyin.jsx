...
export default function MiniGames_pinyin({ user, allMasterCards, selectedIds, timerSetting, setPage }) {
    ...
    if (!gameStarted && user && user.id) {
      ...
      const fetchReviewDisplay = async () => {
        try {
          ...
          console.log('[MiniGames_pinyin] Review poolIds:', poolIds.length);
          
          // ใช้ try-catch สำหรับ localStorage (อาจมีปัญหาในบาง browser)
          let reviewPlayedWords = [];
          try {
            const reviewStorageKey = `playedWords_pinyin_review`;
          ...
          console.log('[MiniGames_pinyin] Review playedWords:', reviewPlayedWords.length);
          ...
          console.log('[MiniGames_pinyin] Review calculation:', {
          ...
          console.log('[MiniGames_pinyin] Setting reviewCountDisplay:', displayCount);
  ...
  const handleAnswer = async (selected) => {
    ...
    if (isCorrect) {
      ...
      // กรณี Review Mode
      if (mode === 'review') {
        // อัพเดท minigame_wrong_count แยกตาม game_type 'pinyin' (reset เป็น 0)
  ...
  if (!gameStarted) {
    ...
    const normalStorageKey = `playedWords_pinyin_normal`;
