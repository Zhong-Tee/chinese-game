...
export default function ClassSchedule({ user, setPage }) {
  ...
  return <div className="min-h-full bg-[#f5f7ff] text-slate-800 -m-4 p-4 pb-12 select-none" style={{ paddingTop: 'max(1rem, calc(env(safe-area-inset-top) + 0.75rem))', paddingLeft: 'max(1rem, calc(env(safe-area-inset-left) + 0.5rem))', paddingRight: 'max(1rem, calc(env(safe-area-inset-right) + 0.5rem))' }}>
    ...
    <div className="flex items-end justify-between mb-3"><div><p className="text-xs font-bold text-indigo-500">{day === todayDayId ? 'วันนี้' : 'ตารางเรียน'} · {saveStatus === 'loading' ? 'กำลังโหลด...' : saveStatus === 'saving' ? 'กำลังบันทึก...' : saveStatus === 'error' ? 'บันทึกไม่สำเร็จ' : 'บันทึกบน Supabase แล้ว'}</p><h2 className="text-2xl font-black">{WEEK_DAYS.find((x) => x.id === day).label}</h2></div><button disabled={!dataReady} onClick={openNew} className="rounded-2xl bg-indigo-600 disabled:bg-slate-300 text-white px-4 py-2.5 font-black shadow-lg shadow-indigo-200">＋ เพิ่มวิชา</button></div>
