...
const OUTFIT_OPTIONS = [
  'ชุดนักเรียน กางเกงนักเรียน',
  'ชุดพละ กางเกงพละ',
  'ชุด stem สีชมพู กางเกงพละ',
  'ชุดลำลอง',
  'ชุดลูกเสือ',
  'เสื้อสี กางเกงพละ',
];
const dateKey = (date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
const nextDateForDay = (dayId) => {
  const date = new Date();
  const targetDay = WEEK_DAYS.findIndex(({ id }) => id === dayId) + 1;
...
export default function ClassSchedule({ user, setPage }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setToday(dateKey(new Date())), nextMidnight.getTime() - Date.now());
    return () => clearTimeout(timer);
  }, [today]);
  ...
  const selectedOutfit = data.outfits?.[day] || '';
  const savedSpecialOutfit = data.specialOutfits?.[day];
  const selectedDayDate = nextDateForDay(day);
  const activeSpecialOutfit = savedSpecialOutfit?.date === selectedDayDate && savedSpecialOutfit.date >= today ? savedSpecialOutfit : null;
  const selectOutfit = (outfit) => {
    setSaveStatus('saving');
    setData((old) => ({
      ...old,
      outfits: { ...old.outfits, [day]: outfit },
      specialOutfits: { ...old.specialOutfits, [day]: null },
    }));
  };
  const selectSpecialOutfit = (outfit) => {
    setSaveStatus('saving');
    setData((old) => ({
      ...old,
      specialOutfits: { ...old.specialOutfits, [day]: { outfit, date: selectedDayDate } },
    }));
  };
  const save = (e) => {
    e.preventDefault();
