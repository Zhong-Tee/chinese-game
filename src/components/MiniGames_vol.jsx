...
export default function MiniGames_vol({ user, allMasterCards, selectedIds, timerSetting, setPage }) {
  ...
  const handleAnswer = async (selected) => {
    ...
    if (isCorrect) {
      ...
      setShowFeedback(true);
      playSound('wrong');
      saveWrongWord(user.id, questionId, 'vol', currentQuestion);
    ...
    >
      <div className="w-full flex justify-between items-center mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <button onClick={async () => { const qId = currentQuestion?.id1 ?? currentQuestion?.id; if (qId != null) await saveWrongWord(user.id, qId, 'vol', currentQuestion); setWrongWordToast('ได้เพิ่มคำผิดไว้ใน list ให้แล้ว ดูรายการได้ที่ Settings'); setTimeout(() => setWrongWordToast(null), 2500); }} className="bg-amber-500 text-white px-2 py-1 rounded-full font-black text-[10px] italic uppercase">คำผิด</button>
        </div>
