...
export default function MiniGames_vol({ user, allMasterCards, selectedIds, timerSetting, setPage }) {
  ...
  const fetchReviewCount = async () => {
    ...
    // ถ้า minigame_wrong_count เป็น JSON object ให้เช็ค game_type 'vol'
  ...
  const initGame = useCallback(async () => {
    // ดึงคะแนนสะสม
    const { data } = await supabase.from('user_scores').select('total_score').eq('user_id', user.id).eq('game_type', 'vol').single();
    ...
    if (mode === 'review') {
      ...
      // กรองเฉพาะคำที่มี minigame_wrong_count > 0 สำหรับ game_type 'vol'
  ...
  useEffect(() => {
    ...
    if (!gameStarted && user && user.id) {
      console.log('[MiniGames_vol] useEffect triggered, user.id:', user.id);
      
      // คำนวณ reviewCountDisplay ทันที (ไม่ต้องรอคลิก Review)
      const fetchReviewDisplay = async () => {
        try {
          console.log('[MiniGames_vol] Starting fetchReviewDisplay...');
          ...
          if (error) {
            console.error('[MiniGames_vol] Error fetching review count:', error);
          ...
          if (!reviewData) {
            console.warn('[MiniGames_vol] No review data returned');
            setReviewCountDisplay(0);
            setReviewCount(0);
            return;
          }
          ...
          try {
            ...
            console.error('[MiniGames_vol] Error reading localStorage:', e);
            reviewPlayedWords = [];
          }
          ...
          console.error('[MiniGames_vol] Error in fetchReviewDisplay:', error);
          // Set fallback value
          setReviewCountDisplay(0);
          setReviewCount(0);
        }
      ...
      console.log('[MiniGames_vol] User not ready yet, waiting...');
    }
  ...
  const handleAnswer = async (selected) => {
    ...
    if (isCorrect) {
      ...
      // อัพเดท minigame_wrong_count แยกตาม game_type 'vol'
      let newWrongCount = {};
      if (typeof prog?.minigame_wrong_count === 'object' && prog?.minigame_wrong_count !== null) {
        newWrongCount = { ...prog.minigame_wrong_count, vol: (prog.minigame_wrong_count.vol || 0) + 1 };
      } else {
        // ถ้าเป็น number เก่า ให้แปลงเป็น object
        const oldCount = prog?.minigame_wrong_count || 0;
        newWrongCount = { vol: oldCount + 1 };
      }
    ...
    return (
      ...
      >
        <div className="text-center mb-8">
          <h1 className="text-4xl md:text-6xl font-black italic uppercase text-purple-600 mb-4">เกมเติมคำ</h1>
        ...
        <button 
          ...
          className="bg-purple-500 text-white px-12 md:px-16 py-4 md:py-6 rounded-[2rem] shadow-xl font-black text-xl md:text-3xl italic uppercase transform active:scale-95 transition-all"
        >
          🎮 Start Game
        </button>
